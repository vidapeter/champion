...
void PM_dev::BuildPollCommand()
{
	...
	*cmdBufPtr++ = 1; // the number of pm-specific command bytes to follow
...
void PM_dev::BuildResetCommand(){
	
	for (int i = 0; i < sizeof(rstBuf); i++)
		rstBuf[i] = 0;
	rstLen = 0;
	uint8_t *cmdBufPtr = &rstBuf[0];
	...
	*cmdBufPtr++ = 0x86; //gofinished

	*cmdBufPtr++ = 0x24; //set workout
	*cmdBufPtr++ = 0x02; //2 data bytes following
	*cmdBufPtr++ = 0x03; // 4. workout: 30 min rowing
	*cmdBufPtr++ = 0x00; //DontCare
	
	*cmdBufPtr++ = 0x86; //gofinished
	*cmdBufPtr++ = 0x81; //reset state machinie
	*cmdBufPtr++ = 0x82; // goidle state machine
	*cmdBufPtr++ = 0x85; // goinuse state machine
...
uint8_t PM_dev::SendResetCommand(uint8_t *count) {
#if 0
	Notify(PSTR("send: "));
	for (uint8_t i = 0; i < rstLen; i++)
	{
		PrintHex<uint8_t>(rstBuf[i]);
		Serial.print(" ");
	}
	Serial.println("");
#endif
	...
	rcode = pPmUsb->SendCommand(rstBuf, PM_REPORT_LEN);
	if (rcode != 0) {
#if 0
