...
int8_t MAX3421e< SS, INTR >::Init()
{
	...
	{ //OSCOKIRQ hasn't asserted in time
                // RLB ignore this error
			//Serial.println("Reset is problem");
		return ( -1 );
	}
        //RLB
        //delay(100);
	regWr( rMODE, bmDPPULLDN|bmDMPULLDN|bmHOST );      // set pull-downs, Host
        //Serial.print("rMode "); Serial.println(regRd(rMODE), HEX);
  
	regWr( rHIEN, bmCONDETIE|bmFRAMEIE );                                             //connection detection
        //Serial.print("rHIEN "); Serial.println(regRd(rHIEN), HEX);

    /* check if device is connected */
    //Serial.print("rHCTL "); Serial.println(regRd(rHCTL), HEX);
    regWr( rHCTL,bmSAMPLEBUS );                                             // sample USB bus
    //Serial.print("rHCTL "); Serial.println(regRd(rHCTL), HEX);
    //delay(10);
    // Serial.print("rHCTL "); Serial.println(regRd(rHCTL), HEX);
    // RLB this isn't right - it never loops, and the chip never seems to clear the bit...
    while(!(regRd( rHCTL ) & bmSAMPLEBUS ));                                //wait for sample operation to finish
    //Serial.print("rHCTL "); Serial.println(regRd(rHCTL), HEX);

	busprobe();                                                             //check if anything is connected
    //Serial.print("rHCTL "); Serial.println(regRd(rHCTL), HEX);
   // Serial.print("rHIRQ "); Serial.println(regRd(rHIRQ), HEX);
    regWr( rHIRQ, bmCONDETIRQ );                                            //clear connection detect interrupt                 
    //Serial.print("rCPUCTL "); Serial.println(regRd(rCPUCTL), HEX);
