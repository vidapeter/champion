...
nak_limit = 0;
//Notify(PSTR("USB::inTransfer nbytes "));
//Serial.print(*nbytesptr, DEC);
//Notify(PSTR(", nak_limit "));
//Serial.println(nak_limit, DEC);
	if (rcode)
		return rcode;
...
{
//Notify(PSTR("USB::InTransfer nak_limit ")); Serial.println(nak_limit, DEC);
	uint8_t rcode = 0;
	uint8_t pktsize;
...
uint8_t USB::dispatchPkt( uint8_t token, uint8_t ep, uint16_t nak_limit )
{
	...
	while( timeout > millis() ) 
	{
		...
		case hrTIMEOUT:
			...
			break;
		default:
                        //Serial.print(nak_count, DEC);
                        //Serial.print(' ');
                        //Serial.println(millis() - now, DEC);
			return( rcode );
		}//switch( rcode
...
void USB::Task( void )      //USB state machine
{
    ...
    switch( tmpdata ) 
	{
        ...
        case SE0:   //disconnected
			...
			lowspeed = false;
			//Serial.println("SE0 disconnected");
            break;
        case LSHOST:
			lowspeed = true;
			//Serial.println("LSHOST");
...
uint8_t USB::Configuring(uint8_t parent, uint8_t port, bool lowspeed)
{
	// RLB not used
        ...
        Notify(PSTR(">USB::Configuring parent ")); Serial.print(parent, HEX);
	...
	{
                Notify(PSTR(" USB::Configuring device index ")); Serial.println(devConfigIndex, DEC);
		if (!devConfig[devConfigIndex])
			continue;
		rcode = devConfig[devConfigIndex]->Init(parent, port, lowspeed);
                Notify(PSTR(" USB::Configuring dev->Init returned ")); Serial.println(rcode, HEX);

		if (!rcode)
		{
			devConfigIndex = 0;
                        Notify(PSTR("<USB::Configuring success\r\n"));
			return 0;
		}
		if (!(rcode == USB_DEV_CONFIG_ERROR_DEVICE_NOT_SUPPORTED || rcode == USB_ERROR_CLASS_INSTANCE_ALREADY_IN_USE))
		{
			...
			if (rcode != USB_DEV_CONFIG_ERROR_DEVICE_INIT_INCOMPLETE)
				devConfigIndex = 0;
                        ...
                        Notify(PSTR("<USB::Configuring returns error ")); Serial.println(rcode, HEX);
			return rcode;
		}
	}
	...
	devConfigIndex = 0;
        ...
        //Notify(PSTR(" USB::Configuing calling DefaultAddressing \r\n"));
	rcode = DefaultAddressing(parent, port, lowspeed);

        Notify(PSTR("<USB::Configuring\r\n"));
	return rcode;
}
